<template>
  <div class="unroll"
    ...
    <div class="item"
      type="button"
      @click="() => { active = false; me!!.state = 1 }">
      <OnlineStatusSymb height="20px"
        class="icon" /> Online
    </div>
    <div class="item"
      type="button"
      @click="() => { active = false; me!!.state = 2 }">
      <IdleStatusSymb height="20px"
        class="icon" /> Idle
    </div>
    <div class="item"
      type="button"
      @click="() => { active = false; me!!.state = 3 }">
      <DoNotDisturbStatusSymb height="20px"
        class="icon" /> DND
    </div>
    <div class="item"
      type="button"
      @click="() => { active = false; me!!.state = 0 }">
      <OfflineStatusSymb height="20px"
        class="icon" /> Offline
    </div>
  </div>
  <div class="self"

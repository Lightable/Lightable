...
<div class="friends">
     <SideContextMenu>
          ...
          <template v-slot:body>
              ...
              <div class="requests">
                  <span class="title" v-if="requests.size >= 1">Requests</span>
                   <FriendItem v-for="(pend, index) in requests" :user="pend[1]" v-bind:key="index" :last="false" :request="true"/>
              </div>
          </template>
        </SideContextMenu>
</div>
...
import FriendItem from "./_cards/FriendItem.vue";
...
export default defineComponent({
    ...
    setup() {
        ...
        return {
            ...
            users: computed(() => clientStore.users),
            pending: computed(() => clientStore.pending),
            requests: computed(() => clientStore.requests)
        }
    },
    mounted() {
       let clientStore = ClientStore();
